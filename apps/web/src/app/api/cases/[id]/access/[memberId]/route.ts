...
export async function PUT(_req: NextRequest, { params }: { params: Promise<{ id: string; memberId: string }> }) {
  ...
  const target = await prisma.member.findUnique({ where: { id: (await params).memberId } });
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string; memberId: string }> }) {
  ...
  await prisma.$transaction(async (tx) => {
    await tx.caseAccess.deleteMany({ where: { caseId: kase.id, memberId: (await params).memberId } });
    await logAction(
      {
        organizationId: owner.organizationId,
        memberId: owner.memberId,
        ...
        targetType: "case",
        targetId: kase.id,
        metadata: { revokedMemberId: (await params).memberId },
      },
      tx
    );
  });
