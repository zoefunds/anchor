...
| Replay attempt (same decision content, fresh message) | Second Sepolia dispatch tx `0x67762c7dccf50626ccfc38c0d4c209b7125633441e1214e34a0f2f4e85674e70` (message ID `0xb6d6bf41fe994f81684b2dd15f3cc5a7f4ec46beb681088cdb2f091713b9a172`), same decisionHash. **Result, honestly stated**: over 20+ minutes of monitoring, no `process()` transaction for this second message ever appeared in `decision-relay`'s on-chain transaction history (checked directly and repeatedly via `solana transaction-history`) — neither a successful nor a failed one. The relayer's own logs showed no explicit skip/simulation-failure message either. **This means replay rejection is not directly proven by a failed on-chain transaction** — what IS directly proven: the ReplayGuard PDA's state remained completely unchanged (`next_index` still exactly 1, only slot 0 populated, byte-for-byte identical) for the entire monitoring window despite the second dispatch existing on Sepolia. That's real, but weaker, evidence — consistent with correct rejection, also consistent with the relayer simply never having gotten around to attempting `process()` on this second message before this pass's monitoring ended. |
...
touches escrow). The third (replay rejection) has real supporting
evidence (the PDA never double-recorded) but not the stronger direct
proof of an observed on-chain transaction failure — that would need
either more monitoring time or a more direct way to force/observe the
relayer's second attempt. Flagged here rather than rounded up to "fully
proven."
